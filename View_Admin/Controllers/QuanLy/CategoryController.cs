...
namespace View_Admin.Controllers.QuanLy
{
    public class CategoryController : Controller
    {
        ...
        {
            string requesURL = $"https://localhost:7268/api/Category";
        ...
        {
            var url = $"https://localhost:7268/api/Category/{id}";
            var httpClient = new HttpClient();
        ...
        public async Task<ActionResult> Create(Category a)
        {
            try
            {
                var url = $"https://localhost:7268/api/Category/Add";
                var httpClient = new HttpClient();
        ...
        public async Task<ActionResult> Edit(Guid id)
        {
            if (ModelState.IsValid)
            {
                ...
                var response = await httpClient.GetAsync(url);
                var dataApi = await response.Content.ReadAsStringAsync();
                var lst = JsonConvert.DeserializeObject<Category>(dataApi);
                return View(lst);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(Category a)
        {
            try
            {
                var url = $"https://localhost:7268/api/Category/Update";
                var httpClient = new HttpClient();
                var content = new StringContent(JsonConvert.SerializeObject(a), Encoding.UTF8, "application/json");
        ...
        {
            var url = $"https://localhost:7268/api/Category/delete/{id}";
            var httpClient = new HttpClient();
