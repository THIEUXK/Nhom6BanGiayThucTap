...
namespace View_Admin.Controllers.QuanLy
{
    ...
    public class ImageController : Controller
    {
        ...
        {
            string requesURL = $"https://localhost:7268/api/Image";
            var httpClient = new HttpClient();
            var response = await httpClient.GetAsync(requesURL);
            string apiData = await response.Content.ReadAsStringAsync();
            var lst = JsonConvert.DeserializeObject<List<Image>>(apiData);
            return View(lst);
        }
        ...
        {
            var url = $"https://localhost:7268/api/Image/{id}";
            var httpClient = new HttpClient();
            var response = await httpClient.GetAsync(url);
            var dataApi = await response.Content.ReadAsStringAsync();
            var lst = JsonConvert.DeserializeObject<Image>(dataApi);
            return View(lst);
        }
        ...
        public async Task<ActionResult> Create(Image a)
        {
            try
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(Image a)
        {
            try
            {
                var url = $"https://localhost:7268/api/Image/Update";
                var httpClient = new HttpClient();
                var content = new StringContent(JsonConvert.SerializeObject(a), Encoding.UTF8, "application/json");
        ...
        {
            var url = $"https://localhost:7268/api/Image/delete/{id}";
            var httpClient = new HttpClient();
